...
namespace fapp_planner
{
  ...
  void PolyTrajOptimizer::addPVAJGradCost2CT(EIGENVEC &gdT, Eigen::VectorXd &costs, const int &K)
  {
    ...
    for (int i = 0; i < N; ++i)
    {
      ...
      for (int j = 0; j <= K; ++j)
      {
        ...
        gradt = 0;
        grad_prev_t = 0;
        for (auto state : objects_->states)
        {
          double t_gap_ = t_now_ - state.header.stamp.toSec() + 0.12;
          ...
          obj_pos += (curT + t_gap_) * obj_vel;
          double dist  = 1.96 - (pos - obj_pos).head(2).squaredNorm();
          if (dist > 0)
          {
            costp += 8000 * dist * dist * dist;
    ...
    // std::cout << "costs(1): " << costs(1) << std::endl;
    costs(3) += var;
  }
  ...
  void PolyTrajOptimizer::setObjects(obj_state_msgs::ObjectsStates *objs_ptr) { objects_ = objs_ptr; }
