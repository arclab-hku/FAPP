...
namespace fapp_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    obj_sub_ = nh.subscribe<obj_state_msgs::ObjectsStates>("/states", 100, &EGOReplanFSM::objectCallback, this);
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    case EXEC_TRAJ:
    {
      ...
      else if ((t_cur > info->duration - 1e-2) && touch_the_goal) // case 3: the final waypoint reached
      {
        ...
        else if (flag_hover_avoid)
        {
          planNextWaypoint(hover_pos_);
          // flag_hover_avoid = false;
        }
        else
        {
      ...
      else if (t_cur > replan_thresh_ || (!touch_the_goal && close_to_current_traj_end)) // case 3: time to perform next replan
      {
        ...
        if ((odom_pos_ - hover_pos_).norm()>0.4) {
          planNextWaypoint(hover_pos_);
          flag_hover_avoid = false;
        }
      }
  ...
  void EGOReplanFSM::checkCollisionCallback(const ros::TimerEvent &e)
  {
    ...
    //   double t_step = 0.1; 
    //   Eigen::Vector3d hover_pt = odom_pos_;
    //   Eigen::Vector3d mid_pt;
    //   mid_pt.setZero();
    //   for (auto state : planner_manager_->traj_.objs.states)
    //   {
    //     double t_gap = t_now - state.header.stamp.toSec() + 0.1;
    //     Eigen::Vector3d obj_pos(state.position.x, state.position.y, state.position.z);
    //     Eigen::Vector3d obj_vel(state.velocity.x, state.velocity.y, 0);
    //     obj_pos += obj_vel * t_gap;
    //     for (int k = 0; k < 15; ++k) 
    //     {
    //       obj_pos += t_step * obj_vel;
    //       if ((hover_pos_ - obj_pos).dot(obj_vel) > 0 &&
    //           (obj_pos - odom_pos_).head(2).norm() < 1.4)
    //       {
    //         Eigen::Vector3d direction = hover_pos_ - obj_pos;
    //         Eigen::Vector3d v_inter;
    //         v_inter << obj_vel(0)*10, obj_vel(1)*10, 0;
    //         // if (v_inter.dot(direction) < 0)
    //         //   v_inter << obj_vel(1), -obj_vel(0), 0;
            
    //         // v_inter.normalize();
    //         // mid_pt += v_inter;

    //         // if (v_inter.dot(direction) > 0)
    //           mid_pt += v_inter.dot(direction)/(direction.norm() * direction.norm()) * direction;
    //         // mid_pt << -obj_vel(1), obj_vel(0), -obj_vel(2);
    //         // if (mid_pt.dot(direction) < 0)
    //         //   mid_pt << obj_vel(1), -obj_vel(0), -obj_vel(2);
    //         need_avoid = true;
    //         break;
    //       }
  ...
  find_ij_start:;
    ...
    for (size_t i = i_start; i < i_end; ++i)
    {
      for (size_t j = j_start; j < pts_chk[i].size(); ++j)
      {
        ...
        for (auto state : planner_manager_->traj_.objs.states)
        {
          double t_gap = info->start_time - state.header.stamp.toSec() + 0.12;
          Eigen::Vector3d obj_pos(state.position.x, state.position.y, state.position.z);
          Eigen::Vector3d obj_vel(state.velocity.x, state.velocity.y, state.velocity.z);
          obj_pos += (t + t_gap) * obj_vel;
          double dist = (p - obj_pos).norm();
          if (dist < 1.4)
          {
            ROS_WARN("drone %d detect collision with objects, dist=%f", planner_manager_->pp_.drone_id, dist);
            dangerous = true;
            break;
          }
        }
  ...
  bool EGOReplanFSM::planHovering(const Eigen::Vector3d mid_pt)
  {
    bool success = false;
    std::vector<Eigen::Vector3d> one_pt_wps;
    Eigen::Vector3d next_wp = mid_pt;

    Eigen::Vector3d next_wp2 = hover_pos_;
    next_wp2(2) += 0.5;
    // next_wp(2) += 0.3; 
    one_pt_wps.push_back(next_wp);
    // one_pt_wps.push_back(next_wp2);
  ...
  void EGOReplanFSM::objectCallback(const obj_state_msgs::ObjectsStatesConstPtr &msg)
  {
    planner_manager_->traj_.objs = *msg;
  }
